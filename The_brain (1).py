...
#                         close_trade(open_position, signal, currency_pair, close, _open, high, low, vol, date, time, day,
#                                     hour, minute, sentiment_table, sentiment_table_name, pivot, s1, s2, s3, r1, r2, r3,
#                                               currency_pair, close)
#                     else:
                    if datetime.now().hour < 21:
                        ...
                        add_parameters_to_data_dictionary_after_signal(currency_pair, signal["Signal"],
                                                                       ...
                                                                       open_position, _open, high, low, vol, date,
...
#                     else:
                    if datetime.now().hour < 21:
                        ...
                        add_parameters_to_data_dictionary_after_signal(currency_pair, signal["Signal"],
                                                                       ...
                                                                       open_position, _open, high, low, vol, date,
                                                                       time, day, hour, minute, currency_trade_id)
...
def add_parameters_to_data_dictionary_after_signal(currency_pair, signal, stop, limit, open_pos, _open, high, low, vol,
                                                   date, time, day, hour, minute, currency_trade_id):
    data_dictionary[currency_trade_id]["signal"] = signal
    data_dictionary[currency_trade_id]["stop"] = stop
    ...
    data_dictionary[currency_trade_id]["time"] = time
...
def close_trade(open_pos, sig, currency, _close, _open, high, low, vol, date, time, day, hour, minute, sentiment_table, sentiment_table_name, pivot, s1, s2, s3, r1, r2, r3,
                                              currency_pair, close):
    if (len(open_pos) > 0):
        if (len(open_pos[open_pos["currency"] == currency]) == 1):
                ...
                if (open_pos[open_pos["currency"] == currency]['isBuy'][index] == True):
                    ...
                    if datetime.now().hour < 21:
                        ...
                        add_parameters_to_data_dictionary_after_signal(currency, sig["Signal"],
                                                                       ...
                                                                       open_pos, _open, high, low, vol, date, time, day,
                                                                       hour, minute)
...
def check_closed_positions(currency):
    ...
    if currency in data_dictionary:
        if data_dictionary[currency] != {}: 
            if "trade_id" in data_dictionary[currency]:
                if len(closed_positions) > 0 and currency in data_dictionary:
                    if len(closed_positions[closed_positions["tradeId"] == data_dictionary[currency]['trade_id']]) == 1:     
                        index = closed_positions[closed_positions["tradeId"] == data_dictionary[currency]['trade_id']].index[0]
                        if closed_positions[closed_positions["tradeId"] == data_dictionary[currency]['trade_id']]["grossPL"][index] > 0:
                            data_dictionary[currency]['win_loss'] = 1
                        else:
                            data_dictionary[currency]['win_loss'] = 0

                        write_to_csv(
                            currency, data_dictionary[currency]['open'],
                            data_dictionary[currency]['high'],
                            ...
                            data_dictionary[currency]['s3'], data_dictionary[currency]['s2'],
                            data_dictionary[currency]['s1'],
...
def add_signal_params(sentiment_table, sentiment_table_name, pivot, s1, s2, s3, r1, r2, r3, curr, _close, currency_trade_id):
...
def write_to_csv(currency, _open, high, low, close, sentiment, net_long,
                 ...
                 win_loss, volume, date, day, hour, minute, tradeId):
    with open('output.csv', 'a', newline='') as f:
        thewriter = csv.DictWriter(f, fieldnames=field_names)
        thewriter.writerow({"Date": date,
                            "Currency": currency,
                            "Day": day,
                            "Hour": hour,
                            "Minute": minute,
...
data = con.get_open_positions()


# In[13]:


data[data['currency']=="EUR/GBP"]['tradeId'].iloc[-1]


# In[15]:


con.open_trade(symbol="EUR/GBP", is_buy=True, is_in_pips=True, amount=7,
                                       time_in_force='GTC', stop=-7, limit=7,
                                       trailing_step=False, order_type='AtMarket')
