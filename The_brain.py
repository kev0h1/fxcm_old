...
def add_parameters_to_data_dictionary_after_signal(currency_pair, signal, stop, limit, open_pos, _open, high, low, vol,
    ...
    if (len(open_pos[open_pos["currency"] == currency_pair]) == 1):
        ...
        trade_id = open_pos[open_pos["currency"] == currency_pair]['tradeId'][index]
        data_dictionary[currency_pair]["trade_id"] = trade_id
...
def close_trade(open_pos, sig, currency, _close, _open, high, low, vol, date, time, day, hour, minute):
    if (len(open_pos) > 0):
        if (len(open_pos[open_pos["currency"] == currency]) == 1):
...
def add_signal_params(sentiment_table, sentiment_table_name, pivot, s1, s2, s3, r1, r2, r3, curr, _close):
    temp = ""
    if curr in data_dictionary:
        temp = curr + str(1)
    else:
        temp = curr
    data_dictionary[temp] = {"s1": s1, "s2": s2, "s3": s3, "pivot": pivot, "r1": r1,
