...
def main():
    ...
    for currency_pair in pairs:
        ...
        try:
            if signal != {}:
                if signal["Signal"] == "Buy":
                    if datetime.now().hour < 21:
                        ...
                        open_position = con.get_open_positions()
                        currency_trade_id = open_position[open_position['currency'] == currency_pair]['tradeId'].iloc[
                            -1]
            ...
            print("Expected error", sys.exc_info()[0], "value:", sys.exc_info()[1], sys.exc_info()[2])
            send_email("Error on Currency "+ currency_pair+ " Expected error" + str(sys.exc_info()[0]) + "value:" + str(sys.exc_info()[1]) + str(sys.exc_info()[2]),
            "Exceptional Error")
...
def add_parameters_to_data_dictionary_after_signal(signal, _stop, limit, _open, high, low, vol,
                                                   date, _time, day, hour, minute, currency_trade_id):
    ...
    if len(open_pos) > 0 and len(open_pos[open_pos["currency"] == currency]) == 1:
        ...
        if sig['Signal'] == "Sell" and open_pos[open_pos["currency"] == currency]['isBuy'][index] == True:
            ...
            if datetime.now().hour < 21:
                ...
                currency_trade_id = open_pos[open_pos['currency'] == currency_pair]['tradeId'].iloc[
                    -1]
                if currency_trade_id not in data_dictionary:
                    add_signal_params(sentiment_table, sentiment_table_name, pivot, s1, s2, s3, r1, r2, r3,
                                      currency_pair, close, currency_trade_id)
...
def machine_learning_algo:
    data = pd.read_csv('output.csv', index_col=0, parse_dates=True)
    currency_dummies = pd.get_dummies(data, columns=['Currency'])
    day_dummies = pd.get_dummies(currency_dummies, columns=['Day'])
    sentiment_dummies = pd.get_dummies(day_dummies, columns=['Sentiment'])
    buy_sell_dummies = pd.get_dummies(sentiment_dummies, columns=['Buy/Sell'])
