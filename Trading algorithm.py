...
def main():
    try:
        ...
        for currency in pairs:
            ...
            signal = machine_learning_algorithm(data)[-1]
...
def machine_learning_algorithm(DF):
    df= DF.copy()
    ohlc = df.iloc[:,[1]]
    ohlc.columns = ["Close"]
    ohlc['returns'] = np.log(ohlc['Close'] / ohlc['Close'].shift(1))
    ohlc.dropna(inplace=True)
    np.digitize(ohlc['returns'], bins=[0])
    lags = 5
    mu = ohlc['returns'].mean()
    v = ohlc['returns'].std()
    bins = [mu - v, mu, mu + v]
    bins = [0]
    cols = []
    for lag in range(1, lags + 1):
        col = 'lag_{}'.format(lag)
        ohlc[col] = np.digitize(ohlc['returns'].shift(lag), bins=bins)
        cols.append(col)
    ohlc=ohlc.iloc[lags:]
    features = ohlc[cols]
    labels = np.sign(ohlc['returns'])
    model = SVC(C=1)
    model.fit(features, labels)
    ohlc['PRED'] = model.predict(features)
    return ohlc['PRED']
