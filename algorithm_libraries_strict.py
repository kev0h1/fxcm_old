...
field_names = ['Date', 'Currency', 'Day', 'Hour', 'Minute', 'TradeId', 'Open', 'High', 'Low', 'Close', 'Volume',
               ...
               'S1', 'P', 'R1', 'R2', 'R3', 'Stop_loss', 'Take_Profit', 'Mid Stop', 'Mid Stop Passed', 'Position Size', 'Stochastic_K',
...
def write_to_csv(file, currency, _open, high, low, close, sentiment, net_long,
                 ...
                 renko_uptrend,
                 renko_bar_num, macd, obv, atr, adx, mid_stop, mid_stop_passed, pos_size):
...
def iterate_through_open_positions(con, trainer_trades_file, log_file, close, currency_pair):
...
def close_out_of_trend_position(direction_of_trend, currency_pair, open_pos, con):
    if len(open_pos) > 0:
        df = open_pos[open_pos['currency']==currency_pair]
        if len(df) > 0:
            for index, data in df.iterrows():
                if direction_of_trend is 'neutral':
