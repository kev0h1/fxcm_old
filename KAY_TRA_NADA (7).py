...
def send_email(message, subject):
    ...
    try:
        ...
        send_email(str("Expected error" + sys.exc_info()[0] + "value:" + sys.exc_info()[1] + sys.exc_info()[2]),
                   "Exceptional Error")
...
def main():
    ...
    for currency_pair in pairs:
        ...
        date = ohlc.iloc[-1].name.to_pydatetime().strftime("%a %d %b %Y")
            ...
            if signal != {}:
                if (signal["Signal"] == "Buy"):
                    if (len(open_position) > 0):
                        close_sell(open_position, signal, currency_pair, close, _open, high, low, vol, date)
                    else:
                    ...
                    if (len(open_position) > 0):
                        close_buy(open_position, signal, currency_pair, close, _open, high, low, vol, date)
                    else:
                        if datetime.now().hour < 17:
...
def add_parameters_to_data_dictionary_after_signal(currency_pair, signal, stop, limit, open_pos, _open, high, low, vol, date):
...
def close_buy(open_pos, sig, currency, _close, _open, high, low, vol, date):
    if (len(open_pos) > 0):
        if (len(open_pos[open_pos["currency"] == currency]) == 1):
                ...
                if (sig['Signal'] == "Sell"):
                    if datetime.now().hour < 17:
                        ...
                        add_parameters_to_data_dictionary_after_signal(currency, sig["Signal"],
                                                                       ...
                                                                       open_pos, _open, high, low, vol, date)


def close_sell(open_pos, sig, currency, _close, _open, high, low, vol, date):
    if (len(open_pos) > 0):
        if (len(open_pos[open_pos["currency"] == currency]) == 1):
            index = open_pos[open_pos["currency"] == currency].index[0]
        ...
        if tradeId == data_dictionary[currency]['tradeId']:
            if con.get_closed_positions()["grossPL"][0] > 0:
                ...
                write_to_csv(
                    ...
                    data_dictionary[currency]['vol'], data_dictionary[currency]['tradeId']
                )
...
#     field_names = ['Currency', 'Date', 'Time', 'TradeId', 'Open', 'High', 'Low', 'Close', 'Volume', 'Sentiment', 'Net Buy',
#                    'Net Sell', 'S3', 'S2',
