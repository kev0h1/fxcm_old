...
def iterate_through_closed_positions_rsi(con, output_file, trainer_trades_file, log_file):
    closed_positions = con.get_closed_positions()
    df = pd.read_csv(trainer_trades_file)
    if len(closed_positions) > 0:
        try:
            for index, data in df.iterrows():
                ...
                if len(trade_id) > 0:
                    ...
                    index_to_delete = df[df['TradeId'] == t_id].index[-1]
                    df.drop(index_to_delete, inplace=True)
            write_header(trainer_trades_file, field_names3)
            for index, data in df.iterrows():
                write_to_csv_rsi(trainer_trades_file,
                             data['Currency'],
                             data['Open'],
                             data['High'],
                             data['Low'],
