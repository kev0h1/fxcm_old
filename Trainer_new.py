...
while True:
    try:
        ...
        if datetime.today().weekday() != 5 and datetime.today().weekday() != 6 and datetime.now().hour < 22:
            ...
            if datetime.now().hour < 19:
                if not con.is_connected():
                    con = fxcmpy.fxcmpy(access_token=TOKEN, log_level='error', log_file='t_old.txt')
                main()
        else:
            if con.is_connected():
                con.close()
        ...
        if day_of_the_week != 5 and day_of_the_week != 6:
            send_email(str(robot_message), "FXCMPY T(new) Logs")
        time.sleep((60 * 15) - ((time.time() - start_time) % (60 * 15)))  # 15 minute interval between each new execution
    except:
        send_email(
            "FXCMPY T_new Expected error" + str(sys.exc_info()[0]) + "value:" + str(sys.exc_info()[1]) + str(sys.exc_info()[2]),
            "Exceptional Error")
