...
def full_pivot_table():
    ...
    page_content = page.content
    soup = BeautifulSoup(page_content, 'html.parser')
    pivots = soup.find_all("div", {"class": "row flex-md-nowrap dfx-border--b-1 dfx-pivotPointsSinglePoint dfx-pivotPointsTable__row"})
    pivot_dictionary={}
...
def main():
    ...
    for currency_pair in pairs:
        ...
        signal = pivot_with_sentiment_signal(currency_pair, ohlc_df, sentiment, pivot_name, close)
        #print("signal " + signal)
        ...
        if signal != {}:
            if(signal["Signal"] == "Buy"):
                if(len(open_position)>0):
                    ...
                    message = "New long position initiated for " + currency_pair + " \rStop Loss: " + signal['Support'] + " \rTake Profit: " + signal['Resistance']
                    send_email(message, "FXCMPY Trade")
                ...
                if(len(open_position)>0):
                    ...
                    message = "New short position initiated for ", currency_pair + " \rStop Loss: " + signal['Resistance'] + " \rTake Profit: " + signal['Support']
                    send_email(message, "FXCMPY Trade")
    ...
    if(len(open_pos[open_pos["currency"]==currency])==1):
        ...
        if(open_pos[open_pos["currency"]==currency]['isBuy'][index]==True):
            ...
            if(sig['Signal'] =="Sell"):
                ...
                message = "New short position initiated for " +  currency_pair + " \rStop Loss: " + sig['Resistance'] + " \rTake Profit: " + sig['Support']
                send_email(message, "FXCMPY Trade")
    ...
    if(len(open_pos[open_pos["currency"]==currency])==1):
        ...
        if(open_pos[open_pos["currency"]==currency]['isBuy'][index]==False):
            ...
            if(sig['Signal']=="Buy"):
                ...
                message = "New long position initiated for " + currency_pair + " \rStop Loss: " + sig['Support'] + " \rTake Profit: " + sig['Resistance']
                send_email(message, "FXCMPY Trade")
...
def pivot_with_sentiment_signal(curr, df, currency_sentiment, pivot_nm, _close):  
    ...
    if currency_sentiment=="Bullish" and df['sma_fast'][-1] > df['sma_slow'][-1]:
        if (low<pivot and _close>pivot) or (min(df['K'][-1],df['D'][-1]) > 25 and max(df['K'][-2],df['D'][-2]) < 25 and _close>pivot and _close<r1):
            return {"Signal":"Buy", "Support": s1, "Resistance":r1}
        if (low<s1 and _close>s1) or (min(df['K'][-1],df['D'][-1]) > 25 and max(df['K'][-2],df['D'][-2]) < 25 and _close>s1 and _close<pivot):
            return {"Signal":"Buy", "Support": s2, "Resistance":pivot}
    
    elif currency_sentiment=="Bearish" and df['sma_fast'][-1] < df['sma_slow'][-1]:
        if (high>pivot and _close<pivot) or (min(df['K'][-1],df['D'][-1]) > 75 and max(df['K'][-2],df['D'][-2]) < 75 and _close<pivot and _close>s1):
            return {"Signal":"Sell", "Support": s1, "Resistance":r1}
        if (high>r1 and _close<r1) or (min(df['K'][-1],df['D'][-1]) > 75 and max(df['K'][-2],df['D'][-2]) < 75 and _close<r1 and _close>pivot):
            return {"Signal":"Sell", "Support": pivot, "Resistance":r2}
