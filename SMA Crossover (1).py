...
def main():
    ...
    for currency_pair in pairs:
        #try:
        sentiment = get_currency_sentiment()[currency_pair]
        tradeId=""  
        ohlc = get_data(currency_pair, 'm5', 250)
        #signal = trade_signal(ohlc, currency)
        signal = pivot_with_sentiment_signal(currency_pair, ohlc, sentiment)
...
def pivot_with_sentiment_signal(curr, df, currency_sentiment):
    ...
    if close<r1 and currency_sentiment=="Bullish":
        return "Buy"
    
    elif close>s1 and currency_sentiment=="Bearish":
        return "Sell"
