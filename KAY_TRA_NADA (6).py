                ...
                if(signal["Signal"] == "Buy"):
                    if(len(open_position)>0):
                        ...
                        if datetime.now().hour < 17:
                            ...
                            add_parameters_to_data_dictionary_after_signal(currency_pair, signal["Signal"], signal['Support'], signal['Resistance'], open_position, _open, high, low, vol)
                    ...
                    if(len(open_position)>0):
                        ...
                        if datetime.now().hour < 17:
                            ...
                            add_parameters_to_data_dictionary_after_signal(currency_pair, signal["Signal"], signal['Resistance'], signal['Support'], open_position, _open, high, low, vol)
    ...
    if(len(closed_positions[closed_positions["currency"]==currency])==1):
        ...
        if tradeId == data_dictionary[currency]['tradeId']:
            if con.get_closed_positions()["grossPL"][0]>0:
                ...
                write_to_csv(
                currency, time, data_dictionary[currency]['open'], data_dictionary[currency]['high'], 
                    data_dictionary[currency]['low'], data_dictionary[currency]['close'], data_dictionary[currency]['sentiment'], 
                    ...
                    data_dictionary[currency]['pivot'], data_dictionary[currency]['r1'], data_dictionary[currency]['r2'], 
                    data_dictionary[currency]['r3'], data_dictionary[currency]['stop'], data_dictionary[currency]['limit'], 
                    data_dictionary[currency]['signal'], data_dictionary[currency]['win_loss'], data_dictionary[currency]['vol'], data_dictionary[currency]['tradeId']
                )
...
with open('output.csv', 'w', newline='')as f:
    field_names = ['Currency','Time', 'TradeId', 'Open', 'High', 'Low', 'Close', 'Volume', 'Sentiment', 'Net Buy', 'Net Sell', 'S3', 'S2',
                  'S1', 'P', 'R1', 'R2', 'R3', 'Stop_loss', 'Take_Profit', 'Buy/Sell', 'Win/Loss']
    thewriter = csv.DictWriter(f, fieldnames=field_names)
    thewriter.writeheader()


def write_to_csv(currency, time, _open, high, low, close, sentiment, net_buy,
                net_sell, s3, s2, s1, p, r1, r2, r3, stop, limit, order_type, 
                 win_loss, volume, tradeid):      
