...
pairs = ['EUR/USD', 'USD/JPY', 'AUD/USD', 'GBP/USD', 'USD/CAD', 'NZD/USD', 'EUR/GBP', 'EUR/JPY',
         'GBP/JPY', 'USD/CHF', 'GER30', "FRA40", "UK100", "SPX500",
         "USOil"]
...
field_names = ['Date', 'Currency', 'Day', 'Hour', 'Minute', 'TradeId', 'Open', 'High', 'Low', 'Close', 'Volume',
               ...
               'S1', 'P', 'R1', 'R2', 'R3', 'Stop_loss', 'Take_Profit', 'Mid Stop', 'Mid Stop Passed', 'Position Size',
               'Stochastic_K',
               'Stochastic_D', 'SMA_slow',
...
def check_for_trade_id_in_file(trades_file, trade_id):
    df = pd.read_csv(trades_file)
    if len(df[df['TradeId'] == trade_id]) > 0:
        return True
    return False
...
def write_to_csv(file_name, currency, _open, high, low, close, sentiment, net_long,
                 ...
                 renko_uptrend,
                 renko_bar_num, macd, obv, atr, adx, mid_stop, mid_stop_passed, pos_size, resistance_1=None,
                 resistance_1_strength=None, resistance_2=None,
    ...
    with open(file_name, 'a', newline='') as f:
        ...
        writer.writerow({"Date": date,
                         ...
                         "Take_Profit": limit,
                         'Mid Stop': mid_stop,
                         'Mid Stop Passed': mid_stop_passed,
...
def iterate_through_open_positions(con, trainer_trades_file, log_file, close, currency_pair, open_positions):
    df = pd.read_csv(trainer_trades_file)
    if len(open_positions) > 0:
        try:
            for index, data in df.iterrows():
                if data['Currency'] == currency_pair:
                    t_id = data['TradeId']
                    trade_id = open_positions[open_positions['tradeId'] == str(t_id)]
                    if len(trade_id) > 0:
                        if data['Buy/Sell'] == 'Buy':
                            if data['Mid Stop'] < close:
                                df.at[index, 'Mid Stop Passed'] = True

                            if data['Mid Stop Passed'] is True and data['Mid Stop'] > close:
                                pos = con.get_open_position(t_id)
                                pos.close(amount=data['Position Size'])
                                message = currency_pair + " (Trade Id - " + str(t_id) + ") closed at Midpoint"
                        ...
                        if data['Buy/Sell'] == 'Sell':
                            ...
                            if data['Mid Stop Passed'] is True and data['Mid Stop'] < close:
                                pos = con.get_open_position(t_id)
                                pos.close(amount=data['Position Size'])
                                message = currency_pair + " (Trade Id - " + str(
                                    t_id) + ") closed at Midpoint"
                                logger(message, log_file=log_file)
            ...
            message = "From logger -Expected error" + str(sys.exc_info()[0]) + "value:" + str(
                sys.exc_info()[1]) + str(
                sys.exc_info()[2])
            logger(message, log_file=log_file)


def close_out_of_trend_position(direction_of_trend, currency_pair, open_pos, con, log_file):
    try:
        if len(open_pos) > 0:
            df = open_pos[open_pos['currency'] == currency_pair]
            if len(df) > 0:
                for index, data in df.iterrows():
                    if direction_of_trend is 'up' and data['isBuy'] is False:
                        pos = con.get_open_position(data['tradeId'])
                        pos.close(amount=1)
                    if direction_of_trend is 'down' and data['isBuy'] is True:
                        pos = con.get_open_position(data['tradeId'])
                        pos.close(amount=1)
    except:
        message = "From logger -Expected error" + str(sys.exc_info()[0]) + "value:" + str(
...
def iterate_through_closed_positions(con, output_file, trainer_trades_file, log_file, closed_positions):
    df = pd.read_csv(trainer_trades_file)
