...
TOKEN = "2182fc0a6737e7dbd82761461915b5d4d10f2024"
                        ...
                        if len(open_position) >0:
                            if len(open_position[open_position['currency'] == currency_pair]) != 0:
                                ...
                                if currency_trade_id not in data_dictionary:
                                    ...
                                    write_to_csv(file='trainer_trades.csv', currency=currency_pair, _open=_open, high=high,
                                                 low=low, close=close,
...
def iterate_through_closed_positions():
    ...
    df = pd.read_csv('trainer_trades.csv')
    try:
        for index, data in df.iterrows():
            ...
            if len(trade_id) > 0:
                ...
                i = df[df['TradeId']==t_id].index[-1]
                df.drop(i, inplace=True)
        write_header('trainer_trades.csv')
        for index, data in df.iterrows():
            write_to_csv('trainer_trades.csv',
                             data['Currency'],
                             data['Open'],
                             data['High'],
                             data['Low'],
