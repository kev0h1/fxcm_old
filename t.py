...
#         send_email(
#             "FXCMPY_T - Expected error" + str(sys.exc_info()[0]) + "value:" + str(sys.exc_info()[1]) + str(
#                 sys.exc_info()[2]),
#             "Exceptional Error")
...
pairs = ['EUR/USD', 'USD/JPY', 'AUD/USD', 'GBP/USD', 'USD/CAD', 'NZD/USD', 'AUD/JPY', 'EUR/CHF', 'EUR/GBP', 'EUR/JPY',
         'GBP/JPY', 'USD/CHF', 'BTC/USD', "ETH/USD", "LTC/USD", 'GER30']
                ...
                if signal["Signal"] == "Buy":
                    if datetime.now().hour < 21:
                        ...
                        if len(open_position[open_position['currency'] == currency_pair]) != 0:
                            ...
                            if currency_trade_id not in data_dictionary:
                                add_signal_params(sentiment_table=sentiment_table,
                                                  sentiment_table_name=sentiment_table_name,
                                                  pivot=pivot, s1=s1, s2=s2, s3=s3, r1=r1, r2=r2, r3=r3,
                                                  curr=currency_pair, _close=close, currency_trade_id=currency_trade_id)
                                add_parameters_to_data_dictionary_after_signal(signal=signal["Signal"],
                                                                               _stop=signal['Support'],
                                                                               limit=signal['Resistance'],
                                                                               _open=_open, high=high, low=low, vol=vol,
                                                                               date=date,
                                                                               _time=_time, day=day, hour=hour,
                                                                               minute=minute,
                                                                               currency_trade_id=currency_trade_id,
                                                                               stoch_k=stoch_k, stoch_d=stoch_d,
                                                                               sma_slow=sma_slow, sma_fast=sma_fast,
                                                                               uptrend=uptrend,
                                                                               renko_bar_num=renko_bar_num, mac=mac,
                                                                               obv=obv, atr=atr,
                                                                               adx=adx)
                    ...
                    if datetime.now().hour < 21:
                        ...
                        if len(open_position[open_position['currency'] == currency_pair]) != 0:
                            ...
                            currency_trade_id = open_position[open_position['currency'] == currency_pair]['tradeId'].iloc[-1]
                            if currency_trade_id not in data_dictionary:
                                add_signal_params(sentiment_table=sentiment_table, sentiment_table_name=sentiment_table_name,
                                                  pivot=pivot, s1=s1, s2=s2, s3=s3, r1=r1, r2=r2, r3=r3,
                                                  curr=currency_pair, _close=close, currency_trade_id=currency_trade_id)
                                add_parameters_to_data_dictionary_after_signal(signal=signal["Signal"],
                                                                               _stop=signal['Resistance'], limit=signal['Support'],
                                                                               _open=_open, high=high, low=low, vol=vol, date=date,
                                                                               _time=_time, day=day, hour=hour, minute=minute,
                                                                               currency_trade_id=currency_trade_id,
                                                                               stoch_k=stoch_k, stoch_d=stoch_d, sma_slow=sma_slow, sma_fast=sma_fast,
                                                                               uptrend=uptrend, renko_bar_num=renko_bar_num, mac=mac, obv=obv, atr=atr,
                                                                               adx=adx)
        except:
            print("Expected error", sys.exc_info()[0], "value:", sys.exc_info()[1], sys.exc_info()[2])
            logger(str("Expected error"+ sys.exc_info()[0]+ "value:"+ sys.exc_info()[1]+ sys.exc_info()[2]))
...
def close_trade(open_pos, sig, currency, _close, _open, high, low, vol, date, _time, day, hour, minute, sentiment_table,
                ...
                currency_pair, close, pos_size, stoch_k, stoch_d, sma_slow, sma_fast, uptrend, renko_bar_num, mac, obv,
                atr, adx):
    if len(open_pos) > 0 and len(open_pos[open_pos["currency"] == currency]) == 1:
        index = open_pos[open_pos["currency"] == currency].index[0]
        is_buy_value = open_pos[open_pos["currency"] == currency]['isBuy'][index]
        if sig['Signal'] == "Sell":
            if is_buy_value:
                con.close_all_for_symbol(currency)
...
# with open('trainer_new_log.txt', 'w', newline='') as f:
#     f.write("New Trainer Logs")

def logger(message):
    with open('trainer_new_log.txt', 'a', newline='') as f:
        f.write("\n"+message)
    ...
    try:
        ...
        if datetime.today().weekday() != 5 and datetime.today().weekday() != 6 and datetime.now().hour < 22:
            ...
            if datetime.now().hour < 19:
                if first_iteration:
                    write_header('trainer_trades.csv')
...
x = pd.read_csv('output3.csv', index_col='Date')
